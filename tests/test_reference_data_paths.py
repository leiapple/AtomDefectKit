...
def test_traction_separation_reference_curve_lookup_finds_repo_data(tmp_path):
    atoms = Atoms("V2", positions=[[0.0, 0.0, 0.0], [0.5, 0.5, 0.5]], cell=[1.0, 1.0, 1.0], pbc=True)
    ...
    path = workflow._reference_curve_path()

    assert path is not None
    assert path.name == "ts_V_100.csv"
