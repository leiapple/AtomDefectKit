"""Implementation of OCP and direct EqV3 checkpoints via OCPCalculator."""
...
OCP_MODEL_CHOICES = OCP_LEGACY_MODEL_CHOICES + sorted(EQV3_MODEL_URLS)
...
def _resolve_checkpoint_path(model_name: str, local_cache: str) -> str:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install the legacy OCP backend dependencies with fairchem-core<2.0 via `uv sync --extra ocp`."
        ) from exc
...
@register_model(
    ...
    metadata={
        "model_name": {
            "type": "str",
            "choices": OCP_MODEL_CHOICES,
            "description": (
                "OCP pretrained model name, or one of the direct eqV3 checkpoint "
        ...
        "local_cache": {
            "type": "str",
            "description": "Directory used to cache downloaded OCP checkpoints.",
...
def _build(params, device):
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "The 'ocp' backend requires fairchem-core with OCPCalculator support. "
                "Install the EqV3/OCP environment with `uv sync --extra ocp` on Python 3.11."
            ) from exc
    ...
    if model_name in EQV3_MODEL_URLS:
        config_yml = _prepare_ocp_config_from_checkpoint(checkpoint_path)
